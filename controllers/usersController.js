...
exports.login = (req, res) => {
    ...
    db.query(query, [username], (err, results) => {
        ...
        const token = jwt.sign({ id_user: user.id_user, username: user.username }, process.env.JWT_SECRET, { expiresIn: '1h' });
        res.json({ token });
    });
};
...
exports.updateUser = (req, res) => {
    ...
    const hashedPassword = bcrypt.hashSync(password, 10);
    const query = 'UPDATE users SET username = ?, email = ?, password = ? WHERE id_user = ?';
